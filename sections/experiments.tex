...
\begin{equation}
    \operatorname{LS}=1 - \min(\textrm{distance offset}, 300)
\end{equation}
and 
\begin{equation}
    \operatorname{DS}=1 - \min(\textrm{date offset}, \operatorname{INTERVAL})
\end{equation}
...
\end{itemize}
The recent Venezuelan protests against President Nicolas Maduro and the Brazilian Protests during June 2013 against bus fare hike were two significant protests during our period of evaluation. Fig.~\ref{fig:venezuela_feb}\ref{fig:brazil_june} show how well the planned protest model was able to predict the unfolding of events under both situations. 
%Fig %.~\ref{fig:venezuela_violent} showcases the ability of the model to forecast the violent events also.
