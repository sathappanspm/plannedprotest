...
The problem we try to solve here is the identification of  `Calls for Protest/Strike or any Civil Disobedience movements' in Social Media like News, Blogs, Twitter, Facebook, etc. and to predict the date of event and event location upto a city level resolution.An accurately identified `call for protest' is then sent out to an alerting system. An alert is structured as shown in Fig.~\ref{fig:alertstructure}. It is a structured record containing When/Where/Why/Who of the protest and the current date or date at which a forecast is made.
